...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YYASSERT (tok == token::T_EOF || tok == token::T_NUM || tok == token::T_ASSIGN || tok == token::T_ADD || tok == token::T_SUB || tok == token::T_MUL || tok == token::T_DIV || tok == token::T_DT_INT || tok == token::T_DT_FLOAT || tok == token::T_LITERAL_STR || tok == token::T_READ || tok == token::T_WRITE || tok == token::T_SEMICOLON || tok == token::T_COMMA || tok == token::T_LS_BRACKET || tok == token::T_RS_BRACKET || tok == token::T_BEGIN || tok == token::T_END || tok == token::T_REPEAT || tok == token::T_UNTIL || tok == token::T_DO || tok == token::T_WHILE || tok == token::T_IF || tok == token::T_THEN || tok == token::T_ELSE || tok == token::T_LPAR || tok == token::T_RPAR || tok == token::T_LT || tok == token::T_GT || tok == token::T_LEQ || tok == token::T_GEQ || tok == 40 || tok == 41);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_T_LS_BRACKET (location_type l)
      {
        return symbol_type (token::T_LS_BRACKET, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_T_RS_BRACKET (location_type l)
      {
        return symbol_type (token::T_RS_BRACKET, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_T_LEQ (location_type l)
      {
        return symbol_type (token::T_LEQ, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_T_GEQ (location_type l)
      {
        return symbol_type (token::T_GEQ, std::move (l));
      }
#else
      static
      symbol_type
